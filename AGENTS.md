...
| cargo-llvm-cov | `cargo install --locked cargo-llvm-cov` |
| cargo-crap | `cargo install --locked cargo-crap` |
...
> **Layer 3 — CRAP metric gate** (`just crap`): runs after test coverage
> is generated by `cargo llvm-cov`, before Kani (Layer 5). CI fails if
> any function exceeds CRAP threshold 30. NOT skipped by `--fast`.
