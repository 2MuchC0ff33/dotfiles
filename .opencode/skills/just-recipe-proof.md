...
## Source
STANDARDS.adoc §8.1 (lines 2717–2719); xtask proof module (lines 2960–2993)
...
- MANDATE: The xtask proof module MUST invoke Kani with the following flags:
  - `--enable-unstable` (required for some proof features)
  - `--restrict-vtable` (restrict vtable handling for soundness)
  - `--default-unwind 100` (unwind depth for loop unrolling)
  - `--output-format terse` (compact output)
  - `--concrete-playback` (concrete test playback for failed proofs)
...
The xtask proof module (lines 2960–2993) implements:
```rust
pub fn run() -> Result<()> {
