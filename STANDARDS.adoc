...
`scripts/check-deps.nu` is replaced by `nix flake check`, which verifies
the full environment integrity at the Nix level. A thin shim remains for
...
hermetic development shell. A thin shim remains for backwards
compatibility:
...
opencode version: 1.15.5 (https://opencode.ai)
...
|opencode |https://opencode.ai |1.15.5
