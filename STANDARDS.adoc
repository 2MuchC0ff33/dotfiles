...
`rust-toolchain.toml` |Exact version (e.g. `"1.95.0"`)
...
|*helix* (`hx`) |`vim` / `neovim` |Full drop-in. Built-in LSP,
tree-sitter, file picker. No plugin system (by design).
...
|*gg* (`gg`) |`gitui` / git TUI |TUI for jj — interactive log, diff, and
operation browser. |`cargo install --locked gg`
|===
