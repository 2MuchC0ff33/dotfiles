...
nightly *Scope:* All projects under this organization — new development,
...
===== 0.2.6 Data-First Design — Mandate and Checklist

*MANDATE:* Every project SHALL define its data structures before writing
any algorithm. *MANDATE:* Every struct SHALL be evaluated for hot/cold
splitting potential. *MANDATE:* Every collection accessed in hot paths
SHALL use SoA layout.

Checklist (every PR):

* [ ] Are data structures designed before algorithms?
* [ ] Is every struct evaluated for hot/cold splitting?
* [ ] Are hot path collections SoA (not AoS)?
* [ ] Are state machines data tables (not match arms)?
* [ ] Are there any silent defaults or hidden behaviors?
* [ ] Would a new team member understand the data flow in 30 seconds?
* [ ] Is every cache line justified (are we loading more than we need)?
...
*MANDATE:* One of these seven proof tiers on every `pub fn` doc comment:

`++[++PROVED++]++` (Kani) — model-checked with `kani::requires`/`kani::ensures`.
`++[++PROVED-MIRAI++]++` — abstract-interpreted with MIRAI (MIR-level static analysis).
`++[++PROVED-VERUS++]++` — refinement-type proved with Verus (SMT-backed).
`++[++PROVED-CREUSOT++]++` — deductively verified with Creusot (Why3 backend).
`++[++TESTED++]++` — proptest with 10,000+ random cases per property.
Functions annotated `[TESTED]` MUST also pass the CRAP metric gate (§6.3.1).
`++[++LINTED++]++` — standard clippy + rustc warnings as errors (minimum bar for ALL code).
`++[++FFI++_++AUDITED++]++` — unsafe code reviewed by two engineers, every unsafe block has a SAFETY comment.

CI SHALL fail if a pub fn lacks one of these annotations. *SHOULD:* Use
...
|*Rust channel* |nightly (unpinned rolling) |Required by verification tools
(Miri, MIRAI, Creusot) and Edition 2024 features. Pinned dates blocked by
fenix DLC manifest URL format.

|*Cosmopolitan Libc* |FORBIDDEN |See §1.4.4.2 for rationale. Use Zig musl
linker for static cross-platform binaries.
...
| Verification (MIR) | cargo-mirai | 1.1.12
| Verification (SMT) | Verus | 0.2026.05.24
| Verification (WhyML) | cargo-creusot | 0.11.0
| Cargo Helper | cargo-sort | latest
| Cargo Helper | cargo-machete | latest
| Cargo Helper | cargo-llvm-cov | latest
| Cargo Helper | cargo-crap | 0.2.2
...
|Rust toolchain (rustc, cargo, clippy, rustfmt, miri, etc.) |fenix overlay via
`rust-toolchain.toml` |nightly (rolling) 0`"1.95.0"`)
...
|LSP (Rust) |rust-analyzer (via rustup component) |nightly |`rust-analyzer` in flake

|LSP (Nushell) |`nu --lsp` |0.113.0 |`nushell` in flake

|LSP (TOML) |`taplo lsp stdio` |0.10.0 |cargo install (not in flake)

|LSP (Nix) |nixd |2.9.1 |`pkgs.nixd` in flake

|Formatter (Rust) |rustfmt |nightly |`rustfmt` in flake
...
cargo-mirai → MIR static analysis (abstract interpretation, cargo-mirai commands)
cargo-creusot → deductive verification (WhyML backend, cargo creusot commands)
verus     → refined types (SMT-backed proof, verus commands)
cargo-sort → sort Cargo.toml dependencies (workspace-consistent ordering)
....
...
===== 1.4.4.2 Cosmopolitan Libc — FORBIDDEN

[WARNING]
====
*STATUS:* FORBIDDEN. Do not use Cosmopolitan Libc (cosmo) in any project
under this organization.
====

RATIONALE:

. *Zig linker already handles static cross-platform binaries.*
  `cargo-zigbuild` produces fully static musl binaries for any target
  from a single build host. Cosmo adds zero value over this existing
  capability.

. *Cosmo is a binary *distortion*, not a portable binary.*
  Cosmo binaries embed multiple libc implementations (musl, glibc) plus
  an APE loader, tripling binary size and introducing an untested code
  path for every syscall.

. *Verification incompatibility.* Kani, Miri, MIRAI, and other
  verification tools operate on standard Rust MIR. Cosmo's custom
  `_start` and libc shim introduce unverifiable code paths.

. *Ecosystem fragmentation.* Cosmo requires patched build tools
  (cosmo-cc, cosmo-ld) that conflict with Nix hermetic builds. Adding
  Cosmo to a Nix devShell breaks `nix flake check`.

. *Community abandonment risk.* Cosmo is maintained by a single entity
  (Justine Tunney). There is no community governance, no long-term
  support commitment, and no formal verification of its libc shim.

*ALTERNATIVE:* Use `cargo-zigbuild` with the Zig musl linker
(see §7.1–7.2). This is the MANDATE approach. Zig musl produces
genuine static binaries that Kani can prove and Miri can check.
...
===== 6.2.4 Kani Local CI Integration

Kani runs as part of the local correctness pipeline — not via GitHub Actions.
The primary CI surface is `scripts/local-ci.nu` (Layer 5).

[source,nushell]
----
# Run Kani model checking (Layer 5) locally
just ci-layer 5

# Run full pipeline including Kani
just ci

# Skip Kani for fast iteration
just ci-fast
----

Kani is installed via the Nix flake derivation at `nix/kani.nix`.
It provides the pre-built `cargo-kani` binary, `kani-compiler`, and
`kani-driver` for `x86_64-linux`.

[source,bash]
----
# Enter devShell (kani available automatically)
nix develop .
...
cargo kani --default-unwind 100 --output-format terse
----

NOTE: The `.github/workflows/` files retained in the repository are
for hosted PR review context only.
They are NOT the primary correctness gate.
Correctness is enforced by `just ci` before every push.
...
[#crap-metric-gate]
==== 6.3.1 CRAP Metric Gate (cargo-crap)

The CRAP (Change Risk Anti-Patterns) metric combines cyclomatic complexity
and test coverage into a single risk score per function:

....
CRAP(m) = comp(m)² × (1 − cov(m)/100)³ + comp(m)

Where:
  comp(m) = cyclomatic complexity of the function
  cov(m)  = line/branch coverage percentage (0–100)

Threshold: 30  (functions above this score are flagged as high-risk)
....

A function annotated `[TESTED]` (§0.3.3) MUST NOT exceed the CRAP threshold.
This gate enforces that `[TESTED]` annotations are honestly earned:
a function with high cyclomatic complexity must have proportionally
high test coverage to remain below the threshold.

*MANDATE:* Every project SHALL run `cargo crap` as part of the CI pipeline
after tests complete and before Kani proof harnesses run.
*MANDATE:* CI SHALL fail if any function exceeds CRAP threshold 30.
*MANDATE:* Functions annotated `[PROVED]` (Kani) are exempt from the CRAP
gate — formal proof subsumes the complexity+coverage requirement.
*MANDATE:* `cargo llvm-cov` SHALL be used to generate the LCOV coverage
file consumed by `cargo crap`.
*SHOULD:* Run `cargo crap --json` in CI for structured reporting.

The two-command workflow:

[source,bash]
----
# Step 1: generate LCOV coverage data
cargo llvm-cov --lcov --output-path lcov.info

# Step 2: compute and gate CRAP scores
cargo crap --lcov lcov.info
----

RATIONALE: The CRAP metric captures *risk*, not just quantity.
A trivial function with 0% coverage has CRAP score 1 and is not dangerous.
A complex function (cyclomatic complexity 12) with 0% coverage has CRAP
score 156 and IS dangerous.
This aligns with §0.1: every line justifies its existence.
Complex untested code is dead weight that conceals bugs.
...
# Run Verus refinement-type verification
verus-check:
    verus src/

# Run Creusot deductive verification
creusot-check:
    cargo creusot

# Run complete pre-commit check (lint + test + proof + verify + sort + machete)
pre-commit:
    cargo xtask check && cargo sort --check && cargo machete
...
|Rust compiler |https://rustup.rs |*nightly* (Edition 2024)
...
|Nushell |https://www.nushell.sh |0.113.0
...
|cargo-mirai |https://github.com/endorlabs/MIRAI |*1.1.12*

|Miri |https://github.com/rust-lang/miri |nightly (rustup component)

|cargo-creusot |https://github.com/creusot-rs/creusot |*0.11.0*

|Verus |https://github.com/verus-lang/verus |*0.2026.05.24*

|cargo-sort |https://crates.io/crates/cargo-sort |*latest*

|cargo-machete |https://crates.io/crates/cargo-machete |*latest*

|cargo-llvm-cov |https://github.com/taiki-e/cargo-llvm-cov |latest

|cargo-crap |https://github.com/minikin/cargo-crap |0.2.2
